...
local function aplicarColisaoNoChar(char)
...
local function removerColisaoGrupo(char)
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("BasePart") then
            pcall(function()
...
-- Desativa colisão entre jogadores
pcall(function()
    PhysicsService:CollisionGroupSetCollidable("NoPlayerCollision", "NoPlayerCollision", false)
end)

local antiFlingAtivado = false
local heartbeatConexao
local charAddedConexao
local outrosCharsConexao = {}
...
AddToggle(Servidor, {
    ...
    Default = false,
    Callback = function(Value)
        antiFlingAtivado = Value

        if Value then
            -- Aplica no seu personagem
            if LocalPlayer.Character then
                aplicarColisaoNoChar(LocalPlayer.Character)
            end

            -- Atualiza se resetar
            charAddedConexao = LocalPlayer.CharacterAdded:Connect(function(char)
                char:WaitForChild("HumanoidRootPart", 5)
                aplicarColisaoNoChar(char)
            end)

            -- Desativa velocities anormais
            heartbeatConexao = RunService.Heartbeat:Connect(function()
            ...
            -- Remove colisão com jogadores
            for _, plr in pairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr.Character then
                    aplicarColisaoNoChar(plr.Character)
                end
                outrosCharsConexao[plr] = plr.CharacterAdded:Connect(function(char)
                    aplicarColisaoNoChar(char)
                end)
            end
            ...
            -- Desliga
            if heartbeatConexao then
                heartbeatConexao:Disconnect()
                heartbeatConexao = nil
            end
            if charAddedConexao then
                charAddedConexao:Disconnect()
                charAddedConexao = nil
            end

            for _, conn in pairs(outrosCharsConexao) do
                conn:Disconnect()
            end
            outrosCharsConexao = {}

            if LocalPlayer.Character then
                removerColisaoGrupo(LocalPlayer.Character)
            end
            for _, plr in pairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr.Character then
                    removerColisaoGrupo(plr.Character)
                end
            end
        end
    end
